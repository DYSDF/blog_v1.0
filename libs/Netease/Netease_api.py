...
top_list_all = {
    ...
    22: ['云音乐ACG音乐榜', '/discover/toplist?id=71385702']
}
...
def geturl(song):
    ...
    quality += ' {0}k'.format(music['bitrate'] // 1000)
...
class NetEase(object):
    ...
    def return_toplists(self):
        ...
        for i in range(len(top_list_all)):
            temp.append({"id": i, "name": top_list_all[i][0]})
        return temp
    ...
    def user_playlist(self, uid, offset=0, limit=100):
        action = 'http://music.163.com/api/user/playlist/?offset={}&limit={}&uid={}'.format(offset, limit, uid)
        try:
    ...
    def song_lyric(self, music_id):
        action = 'http://music.163.com/api/song/lyric?os=osx&id={}&lv=-1&kv=-1&tv=-1'.format(music_id)
        try:
            data = self.httpRequest('GET', action)
    ...
    def channel_detail(self, channelids, offset=0):
        ...
        for i in range(0, len(channelids)):
            action = 'http://music.163.com/api/dj/program/detail?id={}'.format(channelids[i])
            try:
                data = self.httpRequest('GET', action)
