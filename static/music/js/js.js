...
// 音乐播放控制
require(["DYUtils", "SimpleMusicPlayer"], function (DYUtils, SimpleMusicPlayer) {
    ...
    function renderPlayListPanel() {
        ...
        // 歌曲列表DOM
        playList[songIndex].forEach(function (item, index, array) {
            childStr += "<li id=\"" + index + "\" class='play_list_li'><div class=\"musicName\">" + item.name + "</div><div class=\"musicTime\">" + formatTime(item.duration / 1000) + "</div></li>";
        });
    ...
    function getPlayListJSON(id) {
        if (!playList[id]) {
            var localPlayList = DYUtils.getLocalStorage("playList");
            var localUpdateTime = DYUtils.getLocalStorage("playListUpdateTime");
            var nowTime = new Date().getDate();

            if (!localPlayList || !localPlayList[id] || localUpdateTime < nowTime) {
                DYUtils.ajax({
                    url: "/music/toplist",
                    method: "POST",
                    data: {
                    ...
                    success: function (data) {
                        ...
                        playList[id] = json;
                        DYUtils.setLocalStorage("playList", playList);
                        DYUtils.setLocalStorage("playListUpdateTime", new Date().getDate());

                        songIndex = id;
                        renderPlayListPanel(playList[id]);
    ...
    // 获取歌单JSON
    function getSongListJson(fn) {
        var localSongList = DYUtils.getLocalStorage("songList"),
            localUpdateTime = DYUtils.getLocalStorage("songListUpdateTime"),
            nowTime = new Date().getDate();
        if (!localSongList || localUpdateTime < nowTime) {
            DYUtils.ajax({
                url: "/music/playlist",
                method: "POST",
                ...
                success: function (data) {
                    ...
                    fn(songList);
                    DYUtils.setLocalStorage("songList", data);
    ...
    MusicPlayer.listener("canplay", function (songInfo) {
        if (songInfo) {
